...
interface IUniloanV1Oracle {
    function consult(address tokenIn, uint amountIn, address tokenOut) external view returns (uint amountOut);
}

library UniloanV1Library {
    using SafeMath for uint;

    /// @notice Uniswap Oracle Router used for all 24 hour TWAP price metrics
    IUniloanV1Oracle public constant ORACLE = IUniloanV1Oracle(0xa661F3644A59A5A5D768ee2765794F1cD084403d);
    ...
    function quoteOracle(address collateral, address borrow, uint amount) public view returns (uint amountOut) {
        return ORACLE.consult(collateral, amount, borrow);
    }
...
contract UniloanV1Pair {
    using SafeMath for uint;

    /// @notice EIP-20 token name for this token
    string public constant name = "UniloanV1Pair LP";
    ...
    /// @notice UNI ERC20 token address
    IERC20 public constant UNI20 = IERC20(0xCA35e32e7926b96A9988f61d510E038108d8068e);

    /// @notice WETH ERC20 token address
    IERC20 public constant WETH20 = IERC20(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);
    ...
    /// @notice The factory contract allowed to override base behavior
    ...
    /// @notice 95% quote value for min buffers
    uint public constant MIN = 95000; // 95% quote value
    ...
    function harvest() external {
        ...
        if (_b > 0) {
            UNI20.approve(address(UNIV2R02), _b);

            address[] memory _path0 = new address[](2);
            _path0[0] = address(UNI20);
            _path0[1] = address(WETH20);

            UNIV2R02.swapExactTokensForTokens(_b.div(2), uint(0), _path0, address(this), now.add(1800));

            address[] memory _path1 = new address[](3);
            _path1[0] = address(UNI20);
            _path1[1] = address(WETH20);
            _path1[2] = address(WETH20) == token0 ? token1 : token0;
    ...
    function _withdrawLiquidity(address asset, uint amount) internal returns (uint withdrew) {
        withdrew = UniloanV1Library.calculateLiquidityToBurn(address(pair), asset, amount);
        withdrew = withdrew.mul(BUFFER).div(BASE);

        uint _b = pair.balanceOf(address(this));
        if (_b < withdrew) {
            _withdraw(withdrew.sub(_b));
        }
        pair.transfer(address(pair), withdrew);

        (uint amount0, uint amount1) = IUniswapV2Pair(pair).burn(address(this));

        require(amount0 >= UniloanV1Library.quoteOracle(token1, token0, amount1.mul(MIN).div(BASE)), "Uniloan::_withdrawLiquidity: volatility slippage amount0");
        require(amount1 >= UniloanV1Library.quoteOracle(token0, token1, amount0.mul(MIN).div(BASE)), "Uniloan::_withdrawLiquidity: volatility slippage amount1");

        if (asset == token0) {
            require(amount0 >= amount, "Uniloan::_withdrawLiquidity: slippage amount0");
        } else if (asset == token1) {
            require(amount1 >= amount, "Uniloan::_withdrawLiquidity: slippage amount1");
        }
     ...
     */
    function depositLiquidity() public returns (bool) {
        uint _amount0 = IERC20(token0).balanceOf(address(this));
        uint _amount1 = IERC20(token1).balanceOf(address(this));
        ...
        if (_amount0 < UniloanV1Library.quoteOracle(token1, token0, _amount1.mul(MIN).div(BASE))) {
            return false;
        }
        if (_amount1 < UniloanV1Library.quoteOracle(token0, token1, _amount0.mul(MIN).div(BASE))) {
            return false;
        }
    ...
    function loan(address collateral, address borrow, uint amount, uint outMin) external returns (uint) {
        require(collateral == token0 || collateral == token1, "Uniloan::loan: invalid collateral");
        require(borrow == token0 || borrow == token1, "Uniloan::loan: invalid borrow");
        ...
        _received = _received.sub(_received.mul(FEE).div(BASE));
        uint _ltv = _received.mul(LTV).div(BASE);

        uint _amountOut = UniloanV1Library.quoteMin(address(pair), collateral, borrow, _ltv);
...
contract UniloanV1PairFactory {
    ...
    function deploy(IUniswapV2Pair _pair) external returns (address) {
        ...
        pairs[address(_pair)] = address(new UniloanV1Pair(_pair));
